...
func InsertOrder(houseId, beginDate, endDate, userName string) (int, error) {
	...
	if err := GlobalConn.Raw("select id from users where name = ?", userName).Scan(&userData).Error; err != nil {
...
func GetOrderInfo(userName, role string) ([]*userOrder.OrdersData, error) {
	...
	GlobalConn.Raw("select id from users where name = ?", userName).Scan(&userData)
